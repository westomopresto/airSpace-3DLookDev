...
const BLDmaterial = new THREE.ShaderMaterial({
  ...
  uniforms: { // some parameters for the shader
    time: { value: 0 },
    fill: { value: new THREE.Color(0xc8c8c8) },
    stroke: { value: new THREE.Color(0xa4a4a4) },
    noiseA: { value: false },
    noiseB: { value: false },
    dualStroke: { value: false },
    seeThrough: { value: false },
...
const mesh = new THREE.Mesh(new THREE.Geometry(), BLDmaterial);
scene.add(mesh);
...
const gltfLoader = new THREE.GLTFLoader();
        gltfLoader.load('https://raw.githubusercontent.com/westomopresto/airSpace-3DLookDev/master/lib/models/airSpace_15.glb', (gltf) => {
        const city = gltf.scene;
            city.traverse( ( child ) => {
                if ( child instanceof THREE.Mesh ) {
                    ...
                    else if (child.name.includes("bld_"))
                    {
